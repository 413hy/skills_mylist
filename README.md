...
逐个 skill 的代表性 dry-run 自测记录见 `SELF_TEST_REPORT.md`。逐个 skill 的调用式输入/输出模拟记录见 `CALL_SIMULATION_REPORT.md`。安装到 Codex skills 目录并通过新 `codex exec` 进程重载触发的验证记录见 `INSTALLATION_VALIDATION_REPORT.md`。
